...
class TestBeatMapDropping:
    ...
    def test_last_entry_exceeds_duration(self):
        """Last entry starts at 95% of a 60s video — only 3s left < 4s min."""
        beat_map = [
        ...
        # Entry #5 starts at 57s with only 3s remaining → dropped
        assert len(timings) == 4
        # All returned entries must end within video duration
        for t in timings:
            assert t["start_s"] + t["duration_s"] <= 60.0

    def test_multiple_entries_overflow(self):
        """Multiple entries near the end should all be dropped."""
        beat_map = [
        ...
        # late1 starts at 45s, 5s remaining → fits (clamped to 5s)
        # late2 starts at 48s, 2s remaining → dropped
        # late3 starts at 49.5s, 0.5s remaining → dropped
    ...
    def test_entry_just_under_min_remaining(self):
        """Entry with 3.9s remaining (< 4.0 min) should be skipped."""
        beat_map = [
            {"start_pct": 0.0, "duration_pct": 0.3, "marker": "first"},
            # starts at 56.1s → 3.9s remaining < 4.0s min
    ...
    def test_dropped_count_equals_input_minus_output(self):
        """The number of dropped entries should be traceable."""
        beat_map = [
        ...
        timings = _beat_map_to_timings(beat_map, total_duration_s=60.0)
        dropped = len(beat_map) - len(timings)

        assert dropped == 2  # entries d and e should be dropped
...
class TestBeatMapClamping:
    ...
    def test_clamp_to_min_then_overflow(self):
        """Entry at 48s of 50s video: raw duration 1s → clamped to 4s → overflow.

        After clamping to 4s, start(48) + duration(4) = 52 > 50.
        But remaining = 2s < 4s min → should be skipped.
        """
        beat_map = [
        ...
        # Second entry: starts at 48s, raw dur = 1s → clamped to 4s → 52 > 50
        # remaining = 2s < 4s → skip
        assert len(timings) == 1
...
class TestEvenDistribution:
    ...
    def test_too_many_overlays_for_short_video(self):
        """10 overlays in 30s — per_overlay clamped to 4s, some dropped."""
        timings = _compute_overlay_timing(overlay_count=10, total_duration_s=30.0)

        # 10 overlays * 4s each + 9 gaps * 0.2s = 41.8s > 30s
        # Available: 30 - (9 * 0.2) = 28.2s → per_overlay = max(4, 28.2/10) = 4.0s
        # Fits: floor(30 / (4.0 + 0.2)) ≈ 7 overlays
        assert len(timings) < 10
        assert len(timings) >= 6  # at least some should fit
        for t in timings:
            assert t["start_s"] + t["duration_s"] <= 30.0

    def test_very_short_video(self):
        """15s video with 5 overlays — very constrained."""
        timings = _compute_overlay_timing(overlay_count=5, total_duration_s=15.0)

        # available = 15 - (4 * 0.2) = 14.2 → per_overlay = max(4, 14.2/5) = 4.0
        # 4.0 + 0.2 = 4.2 per slot, 15 / 4.2 ≈ 3.57 → 3 overlays fit
